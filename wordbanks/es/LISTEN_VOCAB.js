// LISTEN_VOCAB.js — Converted to object-array format for Listen tab

function convert(levelObj) {
  const out = [];

  Object.keys(levelObj).forEach(category => {
    const words = levelObj[category];
    words.forEach(spanish => {
      out.push({
        english: "",        // optional: fill later if you want
        spanish,
        category
      });
    });
...
export default {
  ...
  A2: convert({
    ...
    "Numbers": [
      "once","doce","trece","catorce","quince","dieisiete","diecisiete",
